	...
	1: [function(require, module, exports)
	{
		(function($, Drupal)
		{
			...
			var searchMap = new Vue(
			{
				...
				computed: { /* Свойсто зависищае от запроса */
				...
				methods: {
					...
					init: function()
					{
						...
						//Если мы не на странице клиники
						this.all_clinics.forEach(function(item, i, arr)
						{

							if (kid > 0 && item.tid == kid)
							{
								cl = item;
							}

							if (self && "taxonomy/term/" + item.tid == drupalSettings.path.currentPath && document.referrer == 'https://ctoma.ru/clinics')
							{

								var cid = self.$cookie.get('current');
								if (item.tid != cid)
								{

									self.current_clinic = item;
									self.$cookie.set('current', item.tid);
									self.$cookie.set('Drupal_visitor_current', item.tid);
						...
						var zoom = 10;
						var center = [59.94421707628917, 30.338121544921886];
						if (kid != 0 && cl)
						{
							zoom = 16;
							center = [cl.field_h, cl.field_d]
						}
					...
					addPlacemarks2: function(cl)
					{
						...
						this.all_clinics.forEach(function(element, index)
						{
							...
							if (element.tid == cl.tid)
							{
								...
								myPlacemark.events.add('click', function()
								{
									searchMap.select_clinic2(element);
								}); /* Дабавляем метку на карту */
								map.geoObjects.add(myPlacemark);
					...
					/* Выбираем клинику */
					select_clinic2: function(clinic)
					{
					...
					{ /* Определяем запрос к геокодеру */
						//var q = 'https://geocode-maps.yandex.ru/1.x/?geocode=Санкт-Петербург'&lang=ru_RU&results=1&format=json&sco=longlat';
						var q = 'https://geocode-maps.yandex.ru/1.x/?apikey=7b7265ff-50f0-4a97-b6ef-8a491a66195a&geocode=Санкт-Петербург, ' + this.question + '&lang=ru_RU&results=1&format=json&sco=longlat';
